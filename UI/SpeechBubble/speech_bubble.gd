...
@export var max_width := 256
@export var min_width := 50  # Ensure a minimum width
@export var min_height := 30 # Ensure a minimum height
...
func display_text(text_to_display: String) -> void:
	...
	await resized
	custom_minimum_size.x = max(min_width, min(size.x, max_width))
	custom_minimum_size.y = max(min_height, size.y)
